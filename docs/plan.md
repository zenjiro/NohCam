...
- WinUI プレビュー起動後に発生していたフリーズ/クラッシュを修正
  - `FaceTrackerBridge` で blendshape 配列コピー時に上限チェックが無く、バッファ破壊しうる問題を修正
  - `FaceTracker_Track` に `blendshapeCapacity` を追加し、容量内のみコピーする実装へ変更
  - WinUI 側 P/Invoke シグネチャも更新し、配列長を明示的に渡すように変更
- フレーム処理負荷を安定化
  - トラッキング処理の同時実行を禁止（常に1本）
  - トラッキング用ピクセルバッファを再利用して大規模再割り当てを抑制
- カメラ/推定の動作方針を現行仕様に合わせて調整
  - カメラフォーマット選択を「1920x1080 @ 30fps 優先」に変更
  - 姿勢推定のターゲット更新間隔を `33ms`（約30fps）に設定
  - 起動ログに選択フォーマットと Tracking FPS を出力し、実効値を観測可能化
